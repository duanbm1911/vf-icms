...
def cm_checkpoint_update_rule_section(request):
    if request.method == "POST":
        ...
        CheckpointRuleSection.objects.all().delete()
        ...
        if list_site:
            for item in list_site:
                ...
                if rules:
                    for obj in rules:
                        policy = obj[1]
                        policy_obj = CheckpointPolicy.objects.get(policy=policy)
                        layer = policy_obj.layer
    ...
    try:
        ...
        for site in sites:
            ...
            for user in users:
                ...
                vpn_gateway = user.template.vpn_gateway
                if vpn_gateway:
                    vpn_gateway = vpn_gateway.split(",")
                user_list.append({
                    "id": user.id,
                    ...
                    "vpn_policy": user.template.vpn_policy,
